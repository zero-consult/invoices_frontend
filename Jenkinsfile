    ...
    stages {
		...
		stage('Tag version') {
			...
			steps {
                ...
                sh "git push git@github.com:zero-consult/invoices_frontend.git ${env.MAJOR_VERSION}.${env.MINOR_VERSION}.${env.PATCH_VERSION}"
			}
		}
		stage('Build') {
		    agent {
				...
				script {
				    ...
				    docker.withRegistry('http://nexus:8081', 'Nexus') {
				        if(env.BRANCH_NAME != "production") {
				            app = docker.build("docker-releases/invoices_frontend_${env.BRANCH_NAME}:$TAG")
                            app.push("$TAG")
				        } else {
                            app = docker.build("docker-releases/invoices_frontend:$TAG")
                            app.push("$TAG")
                        }
                    }
				}
